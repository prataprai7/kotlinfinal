...
import com.example.proteinhub.presentation.components.ImagePicker
...
fun AddEditMedicineScreen(
    ...
    val categoryOptions = listOf("Protein", "Creatine", "Vitamins")
    ...
    val imagePickerLauncher = rememberLauncherForActivityResult(ActivityResultContracts.GetContent()) { uri: Uri? ->
        uri?.let {
            ...
            medexViewModel.uploadMedicineImage(it, context) { uploadedUrl ->
                uploading = false
                if (uploadedUrl != null) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text(if (isEditing) "Edit Suppliment" else "Add Suppliment") },
                navigationIcon = {
        ...
        Column(
            ...
            Button(
                ...
                Text(if (isEditing) "Save Changes" else "Add Supplement")
            }
            Spacer(modifier = Modifier.height(8.dp))
            if (isEditing) {
                Button(
